...
def launch_setup(context, *args, **kwargs):
    ...
    # Ensure model and engine file paths are set
    if str(context.perform_substitution(model_file_path)) == '':
        raise ValueError('model_file_path is not set.')
    if str(context.perform_substitution(engine_file_path)) == '':
        raise ValueError('engine_file_path is not set.')
    ...
    interleave_to_planar_node = ComposableNode(
        ...
        parameters=[{
            'input_tensor_shape': [network_image_height, network_image_width, MODEL_NUM_CHANNELS]
        }],
        remappings=[
    ...
    container = ComposableNodeContainer(
        ...
        namespace='grounding_dino_container',
        package='rclcpp_components',
        executable='component_container_mt',
        composable_node_descriptions=[
            resize_node, pad_node, image_to_tensor_node,
            normalize_node, interleave_to_planar_node, reshape_node,
            grounding_dino_preprocessor, grounding_dino_inference_node, grounding_dino_decoder_node
        ],
        output='screen'
    )
...
def generate_launch_description():
    launch_args = [
        ...
        DeclareLaunchArgument(
            ...
            default_value='0.5',
            description='The confidence threshold for the detections'),
    ]
