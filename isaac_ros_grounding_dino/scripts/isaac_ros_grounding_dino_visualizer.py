...
class GroundingDinoVisualizer(Node):
    QUEUE_SIZE = 10
    color = (0, 255, 0)
    bbox_thickness = 1
    ...
    def __init__(self):
        super().__init__('grounding_dino_visualizer')
        self._bridge = cv_bridge.CvBridge()
        self._processed_image_pub = self.create_publisher(
            Image, 'grounding_dino_processed_image',  self.QUEUE_SIZE)
...
def main():
    rclpy.init()
    rclpy.spin(GroundingDinoVisualizer())
    rclpy.shutdown()
