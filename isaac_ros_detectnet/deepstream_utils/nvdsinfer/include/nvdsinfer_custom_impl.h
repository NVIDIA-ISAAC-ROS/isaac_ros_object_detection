...
/**
  ...
  *
  * Users can parse any custom model derived from this interface. Instance would
  * be created by a call to @fn NvDsInferCreateModelParser.
  *
  * Implementations should make sure that all member functions are overridden.
...
public:
  ...
  /**
    ...
    * @return NvDsInferStatus indicating if model parsing was successful.
    */
...
#define CHECK_CUSTOM_INSTANCE_MASK_PARSE_FUNC_PROTOTYPE(customParseFunc) \
  static void checkFunc_ ## customParseFunc (NvDsInferInstanceMaskParseCustomFunc func = \
    customParseFunc) \
    {checkFunc_ ## customParseFunc ();}; \
  extern "C" bool customParseFunc (std::vector < NvDsInferLayerInfo > const & outputLayersInfo, \
    NvDsInferNetworkInfo  const & networkInfo, \
...
#define CHECK_CUSTOM_CLASSIFIER_PARSE_FUNC_PROTOTYPE(customParseFunc) \
  static void checkFunc_ ## customParseFunc (NvDsInferClassiferParseCustomFunc func = \
    customParseFunc) \
    {checkFunc_ ## customParseFunc ();}; \
  extern "C" bool customParseFunc (std::vector < NvDsInferLayerInfo > const & outputLayersInfo, \
    NvDsInferNetworkInfo  const & networkInfo, \
...
#define CHECK_CUSTOM_ENGINE_CREATE_FUNC_PROTOTYPE(customEngineCreateFunc) \
  static void checkFunc_ ## customEngineCreateFunc (NvDsInferEngineCreateCustomFunc = \
    customEngineCreateFunc) \
...
/**
  ...
  * the @a pluginFactoryV2 field in the @a pluginFactory union.
  *
  ...
  *  otherwise.
  */
bool NvDsInferPluginFactoryCaffeGet(
  NvDsInferPluginFactoryCaffe & pluginFactory,
...
/**
  ...
  * @param[in] networkInfo       A reference to a network information structure.
