...
{
  // No additional initialization required at this stage
...
NvDsInferDBScan::re_allocate(size_t num_detections)
{
  ...
  m_distanceMatrix.reset(new MatrixXf(m_maxProposals, m_maxProposals));
  // Each row idx refers to an input sample, and each row is a list of neighbor indicies for
  // that sample
  m_neighborListMatrix.reset(new RowMatrixXi(m_maxProposals, m_maxProposals));
  // Matrix indicating neighborship between samples
  m_neighborshipMatrix.reset(new MatrixXb(m_maxProposals, m_maxProposals));
...
intersectionOverUnion(
  const NvDsInferObjectDetectionInfo & boxA,
  const NvDsInferObjectDetectionInfo & boxB)
{
  ...
  float unionArea = (areaA + areaB) - isectArea;
  if (unionArea <= 0.0f) {
    return 1.0f;  // Maximum distance for degenerate boxes
  }
  float iou = isectArea / unionArea;
...
initializeObject(ClusteredObject & object)
{
  ...
  object.numMembers = 0U;
  object.totalConfidence = 0.0f;
}

void
NvDsInferDBScan::clusterObjects(
  NvDsInferObjectDetectionInfo * detections,
...
void
NvDsInferDBScan::buildDistanceMatrix(
  const NvDsInferObjectDetectionInfo input[],
  int32_t inputSize)
{
