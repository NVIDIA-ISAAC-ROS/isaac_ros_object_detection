...
if [ -n "$TENSORRT_COMMAND" ]; then
  ...
  TENSORRT_LIB_DIR="$(readlink -f "$(dirname "${TENSORRT_COMMAND}")/../../../lib/$(uname -p)-linux-gnu/")"
  LD_LIBRARY_PATH="${LD_LIBRARY_PATH}:${TENSORRT_LIB_DIR}"
...
MODELS_DIR="${ISAAC_ROS_WS}/isaac_ros_assets/models/triton/${ASSET_NAME}"
...
function setup_model() {
  ...
  OUTPUT_PATH=${ISAAC_ROS_WS}/isaac_ros_assets/models/triton/$model_name_from_model_link
  ...
  if [ -n "$ISAAC_ROS_DETECTNET_CONFIG" ]; then
    ...
    ISAAC_ROS_DETECTNET_PATH=$(ros2 pkg prefix isaac_ros_detectnet --share)
    export ISAAC_ROS_DETECTNET_PATH
